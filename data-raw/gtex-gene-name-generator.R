...
# gct file path
gct_path = '~/GTEx_Analysis_v6_RNA-seq_RNA-SeQCv1.1.8_gene_reads.gct'
...
# gct file path
gct_path = '~/GTEx_Analysis_v6p_RNA-seq_RNA-SeQCv1.1.8_gene_reads.gct'
# read gene count matrix
x = read_tsv(gct_path, skip = 2, col_types = cols_only(Name = 'c'))
# gene count sanity check
identical(as.integer(scan(gct_path, what = 'complex', nlines = 2)[2]), nrow(x))
...
# gct file path
gct_path = '~/GTEx_Analysis_2016-01-15_v7_RNASeQCv1.1.8_gene_reads.gct'
# read gene count matrix
x = read_tsv(gct_path, skip = 2, col_types = cols_only(Name = 'c'))
# gene count sanity check
identical(as.integer(scan(gct_path, what = 'complex', nlines = 2)[2]), nrow(x))
